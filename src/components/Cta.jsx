...
*Bruno's Family & Cafe* 🐕☕`;
  ...
  const nextStep = () => {
    if (currentStep === 1 && formData.ownerName && formData.phoneNumber && formData.address) setCurrentStep(2);
    else if (currentStep === 2 && formData.petType && formData.petBreed && formData.petName) setCurrentStep(3);
    else if (currentStep === 3 && formData.dropMethod) setCurrentStep(4);
    else if (currentStep === 4 && formData.startDate && formData.endDate) setCurrentStep(5);
    else {
      setSubmitStatus('error');
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-[#8B5E3C] to-[#6B2E2E] px-8 py-6 text-white">
            ...
            <div className="h-2.5 bg-white/30 rounded-full">
              <div className="h-full bg-[#D4A054] rounded-full transition-all duration-500"
          ...
          <div className="p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit}>
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                  <h2 className="text-2xl font-bold flex items-center gap-3"><FaCar className="text-[#D4A054]" /> Service Selection</h2>
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <label key={option.value} className="flex items-start gap-4 p-5 border-2 rounded-2xl cursor-pointer hover:border-[#D4A054] transition-all">
                        <input
                          type="radio"
                          name="dropMethod"
                          value={option.value}
                ...
                <div className="space-y-6">
                  ...
                  {formData.startDate && formData.endDate && calculateDays() > 0 && (
                    <div className="bg-amber-50 p-6 rounded-2xl text-center">
                      <p className="text-xl font-semibold text-[#8B5E3C]">Total Stay: <span className="text-2xl">{calculateDays()} Days</span></p>
                    </div>
                  )}
                </div>
              )}
              ...
              {currentStep === 5 && (
                <div className="space-y-6">
                  ...
                  <textarea name="specialInstructions" placeholder="Special Instructions (medical condition, behavior, etc.)"
                    value={formData.specialInstructions} onChange={handleChange} rows={3}
                    className="w-full px-5 py-4 border rounded-2xl focus:border-[#D4A054]" />

                  <textarea name="feedingPreferences" placeholder="Feeding Preferences (meals per day, diet, etc.)"
                    value={formData.feedingPreferences} onChange={handleChange} rows={3}
                    className="w-full px-5 py-4 border rounded-2xl focus:border-[#D4A054]" />
